...
public:
    ...
    // 물리엔진 update. base를 먼저 update하고 vehicle에 대한 update수행
...
private:
    ...
    void updatePhysics(PhysicsBody& body)
    {
        ...
        // 현재 body의 kinematics 상태 (dt동안의 상태)
        ...
        // 먼저 충돌이 없다는 가정으로 다음 kinematics 계산. body가 만들어내는 force와 torque를 고려!
        ...
        // 충돌이 있다면 충돌을 반영한 다음 Kinematics로 업데이트
    ...
    {
        // 이전 dt 시간 동안에 대한 linear drag 추가
        // drag 벡터 크기는 v^2에 비례. 속도의 반대 방향.
        // 전체 drag = b*v + c*v*v 이지만 첫번째 b*v는 무시. 
        // drag 힘을 찾기 위해서 body 프레임의 크기와 단위 벡터 방향을 찾는다. 
        // 각 drag도 유사하게 계산.
        ...
        for (uint vi = 0; vi < body.dragVertexCount(); ++vi) {
            ...
            // 속도가 너무 낮으면 drag 계산은 안해주고 넘어가자. 바람의 영향도 결국 여기서 drag_force로 계산 넣어줘야함!!
        ...
        // force는 world frame으로 변환하고 토크는 local frame으로 그대로 둔다
    ...
    static Wrench getBodyWrench(const PhysicsBody& body, const Quaternionr& orientation)
    {
        ...
        for (uint i = 0; i < body.wrenchVertexCount(); ++i) {
            ...
            // 무게 중심보다 멀리 떨이져서 힘이 작용하므로  추가적으로 torque를 더해준다.
    ...
    // 충돌이 없다는 가정에서의 다음 kinematics 계산
        ...
        Kinematics::State& next, Wrench& next_wrench)
    {
        ...
        if (body.isGrounded()) {
            ...
            // 속도에 따른 linear drag 추가하기
            // linear, angular 속도를 추가
            //add linear drag due to velocity we had since last dt seconds
            //drag vector magnitude is proportional to v^2, direction opposite of velocity
            // 전체 댕기는 힘 = b*v + c*v*v 이나 b*v는 무시.
            //total drag is b*v + c*v*v but we ignore the first term as b << c (pg 44, Classical Mechanics, John Taylor)
            //To find the drag force, we find the magnitude in the body frame and unit vector direction in world frame
            ...
            // force와 torque에 따른 새로운 가속도 값을 계산
        ...
        if (body.isGrounded()) {
            ...
            // 새로운 각 가속도를 계산. torque를 사용해서 각 가속도를 계산.
            ...
            // 제어기에 버그가 있는 경우 속도가 크게 증가하는 문제. 이런 경우 값을 버리거나 사용 불가한 값으로 설정

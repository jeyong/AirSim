...
private:
    ...
    void updatePhysics(PhysicsBody& body)
    {
        ...
        // 현재 body의 kinematics 상태(이 상태는 마지막 dt 시간 이후에 있었던)
        ...
        // collision이 없었다고 가정하고 먼저 response를 계산
        // body에서 생성되는 force와 torque를 고려할 필요가 있음
        ...
        // collision이 있다면, collision response가 필요한지 확인
        ...
        // collision이 이미 response되었다면 정보를 update할때까지 이 충돌에 대한 response를 하지 않는다.
    ...
    // collision response가 생성되었는지 표시하는 값을 반환
